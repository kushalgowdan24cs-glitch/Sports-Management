...
export const PlayerTopbar: React.FC = () => {
    ...
    <header className="h-16 bg-dark-900 border-b border-dark-700 flex items-center px-4 gap-4 shrink-0">
      ...
      <div className="flex-1 max-w-md">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search your stats, matches, achievements..."
            value={search}
            onChange={e => setSearch(e.target.value)}
            id="player-topbar-search"
      ...
      <div className="flex items-center gap-1.5 ml-auto">
        ...
        <div ref={notifRef} className="relative">
          <button
            ...
            id="player-notifications-bell"
          >
            <Bell size={18} />
            {unreadCount > 0 && (
              <span className="absolute top-1 right-1 w-4 h-4 bg-emerald-500 rounded-full text-[9px] font-bold text-white flex items-center justify-center animate-pulse-slow">
            ...
            <div className="absolute right-0 top-full mt-2 w-80 bg-dark-800 border border-dark-700 rounded-2xl shadow-2xl z-50 animate-slide-up">
              ...
              <div className="py-4 px-4 space-y-3">
                <div className="flex items-start gap-3 p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/15">
                  ...
                  <div>
                    <p className="text-xs font-medium text-white">New achievement unlocked!</p>
                    <p className="text-[11px] text-gray-500 mt-0.5">Man of the Match — Championship Final</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-xl bg-dark-800/50 border border-dark-700">
                  <span className="text-lg mt-0.5">📋</span>
                  <div>
                    <p className="text-xs font-medium text-white">New feedback from Coach</p>
                    <p className="text-[11px] text-gray-500 mt-0.5">Excellent Month — June review posted</p>
                  </div>
                </div>
              </div>
              <div className="border-t border-dark-700 px-4 py-3">
                <p className="text-xs text-gray-600 text-center">Only your notifications are shown</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div ref={profileRef} className="relative">
          <button
            ...
            id="player-profile-menu-btn"
          >
            <Avatar name={user?.name ?? 'Player'} src={(user as { photoUrl?: string })?.photoUrl} size="sm" />
            <div className="hidden sm:block text-left">
              <p className="text-xs font-semibold text-white leading-tight">{user?.name ?? 'Player'}</p>
              <p className="text-[10px] text-emerald-400">Athlete</p>
            </div>
          ...
          {profileMenuOpen && (
            <div className="absolute right-0 top-full mt-2 w-52 bg-dark-800 border border-dark-700 rounded-2xl shadow-2xl z-50 animate-slide-up overflow-hidden">
              <div className="px-4 py-3 border-b border-dark-700">
                ...
                <span className="inline-block mt-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                  Player
                </span>
              </div>
              <div className="py-1">
                <MenuDropItem icon={User} label="My Profile" onClick={() => { navigate(ROUTES.PLAYER.PROFILE); setProfileMenuOpen(false); }} />
                <MenuDropItem icon={Settings} label="Settings" onClick={() => { navigate(ROUTES.PLAYER.SETTINGS); setProfileMenuOpen(false); }} />
