...
export const CaptainTopbar: React.FC = () => {
  ...
  const notifications = [
    { emoji: '📋', title: 'Attendance not marked yet today', sub: 'Mark today\'s attendance for your team' },
    { emoji: '🏆', title: 'Match vs SJCE in 6 days',        sub: 'Confirm player lineup by July 5' },
    { emoji: '⚡', title: '2 players below attendance 75%',  sub: 'Kiran Rao & Suresh Babu need attention' },
  ];
      ...
      <div className="flex items-center gap-1.5 ml-auto">
          ...
          {notifOpen && (
            <div className="absolute right-0 top-full mt-2 w-80 bg-dark-800 border border-dark-700 rounded-2xl shadow-2xl z-50 animate-slide-up">
              <div className="flex items-center justify-between px-4 py-3 border-b border-dark-700">
                ...
                <span className="text-xs text-emerald-400">{notifications.length} alerts</span>
              </div>
              ...
              <div className="border-t border-dark-700 px-4 py-3">
                <p className="text-xs text-gray-600 text-center">Captain team alerts</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div ref={profileRef} className="relative">
          <button
            ...
            id="captain-profile-menu-btn"
          >
            <Avatar name={user?.name ?? 'C'} src={(user as { photoUrl?: string })?.photoUrl} size="sm" />
            <div className="hidden sm:block text-left">
              <p className="text-xs font-semibold text-white leading-tight">{user?.name ?? 'Captain'}</p>
              <p className="text-[10px] text-emerald-400">Captain</p>
            </div>
          ...
          {profileMenuOpen && (
            <div className="absolute right-0 top-full mt-2 w-52 bg-dark-800 border border-dark-700 rounded-2xl shadow-2xl z-50 animate-slide-up overflow-hidden">
              <div className="px-4 py-3 border-b border-dark-700">
                <p className="text-sm font-semibold text-white">{user?.name ?? 'Captain'}</p>
              ...
              <div className="py-1">
                <MenuDropItem icon={User}     label="My Profile" onClick={() => { navigate(ROUTES.CAPTAIN.PROFILE);  setProfileMenuOpen(false); }} />
                <MenuDropItem icon={Settings} label="Settings"   onClick={() => { navigate(ROUTES.CAPTAIN.SETTINGS); setProfileMenuOpen(false); }} />
